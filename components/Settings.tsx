...
import { resetToDefault } from '../utils/resetApp';interface SettingsProps {
  settings: AppSettings;
  onSave: (settings: AppSettings) => void;
}type SettingsTab = 'connection' | 'guide';const Settings: React.FC<SettingsProps> = ({ settings, onSave }) => {
  const [url, setUrl] = useState(settings.googleScriptUrl || '');
  const [showScript, setShowScript] = useState(false);
  const [activeTab, setActiveTab] = useState<SettingsTab>('connection');  const handleSave = () => {
...
var ROOT_FOLDER_ID = '1Gpn6ZSUAUwSJqLAbYMo50kICCufLtLx-';function doPost(e) {
  ...
  lock.tryLock(10000);   try {
    ...
    var startIdx = data.startIdx || 0; var year = timestamp.getFullYear().toString();
...
return ContentService.createTextOutput("success").setMimeType(ContentService.MimeType.TEXT);  } catch (error) {
